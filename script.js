...
async function loadUserProfile() {
  try {
    ...
    updateHeaderAvatar(data);
    
    // Update header user label with display name or email
    supabase.auth.getUser().then(({ data: { user } }) => {
      updateHeaderUserLabel(user, data);
    });
...
async function saveUserProfile() {
  try {
    ...
    const display_name = displayInput.value.trim() || null;
    ...
    if (error) {
      ...
      updateHeaderAvatar(payload);
      
      // Update header user label
      supabase.auth.getUser().then(({ data: { user } }) => {
        updateHeaderUserLabel(user, payload);
      });
...
function initProfileUI() {
  ...
  if (!backdrop || !saveBtn || !changePasswordBtn) {
  ...
  changePasswordBtn.addEventListener("click", async (e) => {
    ...
    try {
      ...
      console.error('[PROFILE] Password reset exception:', err);
...
async function updateStageStatusBanner(currentStage) {
    ...
    if (isNewPlayer) {
        ...
        textEl.textContent = `Welcome back, ${name} — you're on Stage ${currentStage} of 16`;
    }
...
async function onUserSignedIn(user) {
    try {
        ...
        // Position the app at the next unsolved stage before showing the game
        try {
            if (typeof (app || window.contestApp)?.ensureAtNextUnsolved === 'function') {
                (app || window.contestApp).ensureAtNextUnsolved('signed_in');
            }
        } catch (err) {
            console.warn('[NAV] ensureAtNextUnsolved error:', err);
        }

        // Start the contest UI (use centralized starter so Play button and auto-login match)
        try {
            if (!hasAutoStartedGame) {
                ...
                if (typeof startContestForSignedInUser === 'function') {
                    ...
                    console.log('[AUTH] No UI starter found (showGame/startContestForSignedInUser)');
                }
            } else {
    ...
    if (hasWinner) {
        ...
        card.innerHTML = `
            <div class="lb-left">
                ...
                <div class="lb-stage-label">Stage ${stage}</div>
...
async function renderLeaderboardStage16Big(){
    ...
    card.className = 'stage-16-winner-card stage-16-gold-card-base';
    card.setAttribute('role', 'article');
    card.setAttribute('aria-label', 'Stage 16 Grand Prize');

    if (hasWinner) {
        const formattedDate = winner.won_at ? new Date(winner.won_at).toLocaleDateString() : '';
        card.innerHTML = `
            <div class="stage16-icon open">✔</div>
            <div class="stage16-title">Stage 16 · Master Stage</div>
            <div class="stage16-prize">Grand Prize • 100K Miles</div>
            <div class="winner-name">🎉 Winner: ${winner.username || '—'}</div>
            <div class="winner-caption">Congratulations on completing the ultimate challenge!</div>
            <div class="winner-caption">Won ${formattedDate}</div>
        `;
    } else {
        card.innerHTML = `
            <div class="stage16-icon stage-status-locked">🔒</div>
            <div class="stage16-title">Stage 16 · Master Stage</div>
            <div class="stage16-prize">Grand Prize • 100K Miles</div>
            <div class="winner-caption">Winner will be revealed at the end of the challenge.</div>
            <div class="winner-caption">Solve all earlier stages for your shot at the grand prize.</div>
        `;
    }
...
function updateStage16() {
    ...
    if (!adminEnabled) {
        ...
        stage16Card.innerHTML = `
            <div class="stage16-icon stage-status-locked">⏸️</div>
            <div class="stage16-title">Stage 16 · Master Stage</div>
            <div class="stage16-prize">Grand Finale · 100K Miles</div>
            <div class="stage16-caption">One last riddle. One massive prize. Beat the journey and unlock the Master Stage for your shot at 100,000 Turkish Airlines Miles.</div>
            <div class="stage16-status">Opens as your journey unfolds</div>
        `;
